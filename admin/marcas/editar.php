            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <select name="tipo_dolar"
                        ...
                        <option value="blue" <?= $marca['tipo_dolar'] === 'blue' ? 'selected' : '' ?>>Blue Venta</option>
                        <option value="oficial" <?= $marca['tipo_dolar'] === 'oficial' ? 'selected' : '' ?>>Oficial BNA
                        </option>
                    </select>
                </div>
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-4">Recargo
                        Dólar ($)</label>
                    <input type="number" step="0.01" min="0" name="recargo_dolar_pesos"
                        value="<?= htmlspecialchars($marca['recargo_dolar_pesos'] ?? '0') ?>"
                ...
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-4">Recargo
                        Bco. (%)</label>
                    <input type="number" step="0.01" min="0" name="recargo_bancario_porcentaje"
                        value="<?= htmlspecialchars($marca['recargo_bancario_porcentaje'] ?? '0') ?>"
                        class="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-transparent focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 transition-all outline-none font-bold text-gray-900">
                </div>
            </div>
